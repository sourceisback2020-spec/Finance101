...
type BankAccountType = "checking" | "savings" | "brokerage" | "cash";
type ThemePreset = "midnight" | "emerald" | "sunset";
type BackgroundPreset = "aurora" | "mesh" | "orbital" | "topography";
type DensityPreset = "cozy" | "compact";
...
function ensureDb() {
  ...
  const hasColumn = (name: string) => scenarioColumns.some((column) => column.name === name);
...
function createWindow() {
  ...
  if (isDev) {
    const devUrl = process.env.VITE_DEV_SERVER_URL ?? "http://localhost:5381";
    win.loadURL(devUrl);
  } else {
...
ipcMain.handle("transactions:upsert", (_e, tx: Transaction) => {
  db.prepare(`
...
ipcMain.handle("transactions:bulkInsert", (_e, transactions: Transaction[]) => {
  const insert = db.prepare(`
    INSERT OR REPLACE INTO transactions
    (id, date, amount, type, category, merchant, account, note, recurring)
    VALUES (@id, @date, @amount, @type, @category, @merchant, @account, @note, @recurring)
  `);
  const tx = db.transaction((rows: Transaction[]) => rows.forEach((row) => insert.run(row)));
  tx(transactions);
  return true;
});
...
ipcMain.handle("subscriptions:list", () => {
  return db.prepare("SELECT * FROM subscriptions ORDER BY nextDueDate ASC").all() as Subscription[];
});

ipcMain.handle("subscriptions:upsert", (_e, sub: Subscription) => {
  db.prepare(`
...
ipcMain.handle("subscriptions:delete", (_e, id: string) => {
  db.prepare("DELETE FROM subscriptions WHERE id = ?").run(id);
  return true;
});

ipcMain.handle("cards:list", () => {
  return db.prepare("SELECT * FROM credit_cards ORDER BY name ASC").all() as CreditCard[];
});

ipcMain.handle("cards:upsert", (_e, card: CreditCard) => {
  db.prepare(`
...
ipcMain.handle("scenarios:upsert", (_e, scenario: Scenario) => {
  db.prepare(`
...
ipcMain.handle("retirement:list", () => {
  return db.prepare("SELECT * FROM retirement_entries ORDER BY date DESC").all() as RetirementEntry[];
});

ipcMain.handle("retirement:upsert", (_e, entry: RetirementEntry) => {
  db.prepare(`
...
ipcMain.handle("retirement:delete", (_e, id: string) => {
  db.prepare("DELETE FROM retirement_entries WHERE id = ?").run(id);
  return true;
});

ipcMain.handle("banks:list", () => {
  return db.prepare("SELECT * FROM bank_accounts ORDER BY institution ASC, nickname ASC").all() as BankAccount[];
});

ipcMain.handle("banks:upsert", (_e, account: BankAccount) => {
  db.prepare(`
...
ipcMain.handle("banks:delete", (_e, id: string) => {
...
ipcMain.handle("settings:getUiPreferences", () => {
  const row = db.prepare("SELECT value FROM settings WHERE key = ?").get("ui.preferences") as { value?: string } | undefined;
  if (!row?.value) {
    return defaultUiPreferences;
  }
  try {
    return { ...defaultUiPreferences, ...(JSON.parse(row.value) as Partial<UiPreferences>) } as UiPreferences;
  } catch {
    return defaultUiPreferences;
  }
});

ipcMain.handle("settings:setUiPreferences", (_e, prefs: UiPreferences) => {
  db.prepare(`
...
ipcMain.handle("transactions:exportCsv", async () => {
  const rows = db.prepare("SELECT * FROM transactions ORDER BY date DESC").all() as Transaction[];
  const csv = [
    "id,date,amount,type,category,merchant,account,note,recurring",
    ...rows.map((row) =>
      [
...
ipcMain.handle("backup:saveJson", async (_e, payload: string) => {
